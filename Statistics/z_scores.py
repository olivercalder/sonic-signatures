...
def is_nested(vectors):
    nested = False
    for outer_key in vectors:
        for inner_key in vectors[outer_key]:
            if type(vectors[outer_key][inner_key]) == type({}):
                nested = True
            break
        break
    return nested
...
def load_csv(filename):
    ...
    with open(filename, newline='') as in_csv:
        ...
        for row in reader:
            ...
            vectors[char] = {}
            for phoneme in row:
                vectors[char][phoneme] = float(row[phoneme])
...
def build_z_scores(in_csv='', in_json='', nested=False, silent=False, wt=False, wj=False, title='', directory=''):
    ...
    if not silent:
        print_z_scores(z_scores)
    if wt:
        write_csv(z_scores, title, directory)
    if wj:
        write_json(z_scores, title, directory)
    return z_scores
