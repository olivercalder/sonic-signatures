...
function handleMouseOver(d, i) {
    var circ = d3.select(this)  // circle element being modified
        .style("fill", "white")
        .style("stroke", "black");

    var charName = circ.attr("character");
    var score = parseFloat(circ.attr("score")).toFixed(3);

    var graph = d3.select(this.parentNode)  // g element containing circles

    var chart = d3.select(this.parentNode.parentNode)  // svg element containing everything

    var chartID = chart.attr("id");
    var phonIndex = chartID.indexOf("_");
    var phon = chartID.slice(phonIndex + 1);

    var labelID = phon + "_" + charName.replace(".", "-") + "_label";
